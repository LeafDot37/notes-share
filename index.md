---
created: 2026-07-22
tags: [map-of-content, root]
---

# 📚 Study Notes

> 计算机科学核心课程学习笔记

欢迎来到我的学习笔记知识库！本知识库搭建在 GitHub 服务器上，支持关系图谱、双向链接和全文搜索。**使用 GitHub 加速器可获更佳体验。**

## 使用建议

- 🔍 **全文搜索** — 使用左侧搜索框快速查找知识点
- 🗺️ **关系图谱** — 右侧图谱展示笔记间的关联，可拖拽缩放
- 📂 **左侧导航** — 点击章节目录直达 Hub 页面，展开可查看具体知识点
- ⚠️ **若页内链接出现 404**，请使用左侧导航栏重新定位目标页面
- 📱 **移动端** — 自动适配手机屏幕

## 笔记目录

| 科目 | 入口 |
|------|------|
| **CSAPP** | [[CSAPP/CSAPP\|深入理解计算机系统]] |
| **Algorithms** | [[Algorithms/Algorithms\|算法分析与设计]] |
| **Data Structures** | [[Data Structures/Data Structures\|数据结构]] |
| **Discrete Math** | [[Discrete Math/Discrete Math\|离散数学]] |
| **Info Theory** | [[Info Theory/Info Theory\|信息论基础]] |
| **Machine Learning** | [[Machine Learning/Machine Learning\|机器学习]] |
| **NLP** | [[NLP/NLP\|自然语言处理]] |
| **Signals and System** | [[Signals and System/Signals and System\|信号与系统]] |

## 快速链接

- [[知识图谱构建方法论\|贡献者指南 · 笔记规范]]
- [GitHub 仓库](https://github.com/LeafDot37/notes-share)

---

> 笔记内容可能存在疏漏，请以原始教材为准。
